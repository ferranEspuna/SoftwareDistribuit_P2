...
class MatchesModel(db.Model):
    ...
    competition = db.relationship("CompetitionsModel",foreign_keys=[competition_id])
```
...
Filtres avançats:
    ...
    >>> match = MatchesModel.query.filter_by(id=1).filter(MatchesModel.local(name='Karasuno')).first()
    >>> match.competition
